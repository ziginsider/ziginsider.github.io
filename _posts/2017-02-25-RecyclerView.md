...
Говоря <p>отвлеченно</p>, метод `onCreateViewHolder()` создает "бассейн", а метод `onBindViewHolder()` "наполняет бассейн водой". Если каждый раз, когда меняется представление (скролл) не "менять воду в бассейне" полностью т.е. не переопределять содержание всех элементов, которые могут измениться, в `onBindViewHolder()`, то вьюха может выдавать <p>сюрпризы</p> в виде старых значений. 
