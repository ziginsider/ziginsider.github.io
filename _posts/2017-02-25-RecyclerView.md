...
<h6>Говоря</h6> отвлеченно, метод `onCreateViewHolder()` создает "бассейн", а метод `onBindViewHolder()` "наполняет бассейн водой". Если каждый раз, когда меняется представление (скролл) не "менять воду в бассейне" полностью т.е. не переопределять содержание всех элементов, которые могут измениться, в `onBindViewHolder()`, то вьюха <h6>может выдавать</h6> сюрпризы в виде старых значений. 
